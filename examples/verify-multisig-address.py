...
if __name__ == "__main__":
    ...
    parser.add_argument("--descriptor", help="Bitcoin core script descriptor /path/to/file.json", required=True)
    ...
    for pubkey in parts:
        ...
        print("PUBKEY", pubkey)
        xfp, path, xpub, idx = re.match("\[([0-9a-f]+)\*?(.*?)]([0-9A-Za-z]+).*([0-9]+?)", pubkey).groups()
        print('xfp', xfp)
        print('path', path)
        print('xpub', xpub)
        print('idx', idx)
        ...
        for cnt in range(args.limit):
            sec_hex_list.append([xfp, "{}/{}/{}".format(path, idx, cnt+args.offset), child_obj.child(cnt+args.offset).sec().hex()])
        sec_hex_lists.append(sec_hex_list)

    assert len(networks) == 1, "ERROR: multiple networks in descriptor: {}".format(contents)
