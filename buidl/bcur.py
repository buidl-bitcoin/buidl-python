from buidl.bech32 import cbor_encode, cbor_decode, bc32encode, bc32decode, uses_only_bech32_chars
...
def decode_multi_qrgif_to_payload(qr_payloads, require_checksum=True):
    ...
    for cnt, qr_payload in enumerate(qr_payloads):
        x_res, y_res, checksum_res, encoded_payload, err_msg = parse_bcur(string=qr_payload)
        if err_msg:
        ...
        if y is None:
            ...
            if y_res != y:
                return None, None, f"Inconsistent Y value error: expected {y} but got {y_res}"
        if checksum is None:
            checksum = checksum_res
        else:
            if checksum_res != checksum:
                return None, None, f"Inconsistent checksum across qr payloads: {checksum_res} != {checksum}"
...
def encode_to_bcur_qrgif(text, max_size_per_chunk=300, animate=True):
    ...
    for cnt in range(number_of_chunks):
        ...
        resulting_chunks.append(f"ur:bytes/{cnt+1}of{number_of_chunks}/{enc_hash}/{enc[start_idx:finish_idx]}")
